...
/**
 * @ #Crafty
 * @category Core Select a set of or single entities by components or an
 *           entity's ID.
 * 
 * Crafty uses syntax similar to jQuery by having a selector engine to select
 * entities by their components.
 * 
 * @example ~~~ Crafty("mycomponent") Crafty("hello 2D component")
 *          Crafty("hello, 2D, component") ~~~ The first selector will return
 *          all entities that has the component `mycomponent`. The second will
 *          return all entities that has `hello` and `2D` and `component`
 *          whereas the last will return all entities that has at least one of
 *          those components (or). ~~~ Crafty(1) ~~~ Passing an integer will
 *          select the entity with that `ID`.
 * 
 * Finding out the `ID` of an entity can be done by returning the property `0`.
 * ~~~ var ent = Crafty.e("2D"); ent[0]; //ID ~~~
 */
...
/**
 * @ #Crafty Core
 * @category Core Set of methods added to every single entity.
 */
Crafty.fn = Crafty.prototype = {
	...
	init: function(selector) {
		...
		if(typeof selector === "string") {
			...
			for(e in entities) {
				...
				if(and || or) { // multiple components
					...
					for(;i<l;i++) // loop over components
						if(current.__c[comps[i]]) score++; // if component
															// exists add to
															// score
				...
				} else if(current.__c[selector]) this[elem++] = +e; // convert
																	// to int
			}
			...
			this.length = elem; // length is the last index (already
								// incremented)
	 ...
	 * @param componentList -
	 *            A string of components to add seperated by a comma `,`
	 * @sign public this .addComponent(String component1[, .., String
	 *       componentN])
	 * @param component# -
	 *            Component ID to add. Adds a component to the selected entities
	 *            or entity.
	 * 
	 * Components are used to extend the functionality of entities. This means
	 * it will copy properties and assign methods to augment the functionality
	 * of the entity.
	 * 
	 * There are multiple methods of adding components. Passing a string with a
	 * list of component names or passing multiple arguments with the component
	 * names.
	 * 
	 * @example ~~~ this.addComponent("2D, canvas, health");
	 ...
	 * @param componentList -
	 *            List of components that must be added Makes sure the entity
	 *            has the components listed. If the entity does not have the
	 *            component, it will add it.
	 * @see .addComponent
	 */
	 ...
	 *            Component to remove
	 * @param soft -
	 *            Whether to soft remove it (defaults to `true`) Removes a
	 *            component from an entity. A soft remove will only refrain
	 *            `.has()` from returning true. Hard will remove all associated
	 ...
	 * @ #.has
	 * @comp Crafty Core
	 * @sign public Boolean .has(String component) Returns `true` or `false`
	 *       depending on if the entity has the given component.
	 * 
	 * For better performance, simply use the `.__c` object which will be `true`
	 * if the entity has the component or will not exist (or be `false`).
	 */
	has: function(id) {
		return !!this.__c[id];
	},
	 ...
	 * @param map -
	 *            Object where the key is the property to modify and the value
	 *            as the property value Use this method to set any property of
	 *            the entity.
	 * @example ~~~ this.attr({key: "value", prop: 5}); this.key; //value
	 *          this.prop; //5
	 * 
	 * this.attr("key", "newvalue"); this.key; //newvalue ~~~
	 */
	 ...
	 * @ #.toArray
	 * @comp Crafty Core
	 * @sign public this .toArray(void) This method will simply return the found
	 ...
	 * @param delay -
	 *            Amount of milliseconds to execute the method The delay method
	 *            will execute a function after a given amount of time in
	 *            milliseconds.
	 * 
	 * Essentially a wrapper for `setTimeout`.
	 * 
	 * @example Destroy itself after 100 milliseconds ~~~ this.delay(function() {
	 *          this.destroy(); }, 100); ~~~
	 */
	 ...
	 * @param callback -
	 *            Method to execute when the event is triggered Attach the
	 *            current entity (or entities) to listen for an event.
	 * 
	 * Callback will be invoked when an event with the event name passed is
	 * triggered. Depending on the event, some data may be passed via an
	 * argument to the callback function.
	 * 
	 * The first argument is the event name (can be anything) whilst the second
	 * argument is the callback. If the event has data, the callback should have
	 * an argument.
	 * 
	 * Events are arbitrary and provide communication between components. You
	 * can trigger or bind an event even if it doesn't exist yet.
	 * @example ~~~ this.attr("triggers", 0); //set a trigger count
	 *          this.bind("myevent", function() { this.triggers++; //whenever
	 *          myevent is triggered, increment }); this.bind("EnterFrame",
	 *          function() { this.trigger("myevent"); //trigger myeven on every
	 *          frame }); ~~~
	 * @see .trigger, .unbind
	 */
	 ...
	 * @param callback -
	 *            Function to unbind Removes binding with an event from current
	 *            entity.
	 * 
	 * Passing an event name will remove all events binded to that event.
	 * Passing a reference to the callback will unbind only that callback.
	 * @see .bind, .trigger
	 */
	 ...
	 *            Event to trigger
	 * @param data -
	 *            Arbitrary data that will be passed into every callback as an
	 *            argument Trigger an event with arbitrary data. Will invoke all
	 *            callbacks with the context (value of `this`) of the current
	 *            entity object.
	 * 
	 * *Note: This will only execute callbacks within the current entity, no
	 * other entity.*
	 * 
	 * The first argument is the event name to trigger and the optional second
	 * argument is the arbitrary event data. This can be absolutely anything.
	 */
	 ...
	 * @param method -
	 *            Method to call on each iteration Iterates over found entities,
	 *            calling a function for every entity.
	 * 
	 * The function will be called for every entity and will pass the index in
	 * the iteration as an argument. The context (value of `this`) of the
	 * function will be the current entity in the iteration.
	 * @example Destroy every second 2D entity ~~~ Crafty("2D").each(function(i) {
	 *          if(i % 2 === 0) { this.destroy(); } }); ~~~
	 */
	each: function(fn) {
	 ...
	 * @returns Cloned entity of the current entity Method will create another
	 *          entity with the exact same properties, components and methods as
	 *          the current entity.
	 */
	clone: function() {
		var comps = this.__c,
	 ...
	 * @param callback -
	 *            Method to execute if the property is modified Will watch a
	 *            property waiting for modification and will then invoke the
	 ...
	 * @triggers Remove Will remove all event listeners and delete all
	 *           properties as well as removing from the stage
	 */
	destroy: function() {
...
/**
 * Extension method to extend the namespace and selector instances
 */
...
/**
 * @ #Crafty.extend
 * @category Core Used to extend the Crafty namespace.
 */
Crafty.extend({
	/**
	 * @ #Crafty.init
	 ...
	 *            Width of the stage
	 * @param height -
	 *            Height of the stage Starts the `EnterFrame` interval. This
	 *            will call the `EnterFrame` event for every frame.
	 * 
	 * Can pass width and height values for the stage otherwise will default to
	 * window size (see `Crafty.DOM.window`).
	 * 
	 * All `Load` events will be executed.
	 * 
	 * Uses `requestAnimationFrame` to sync the drawing with the browser but
	 * will default to `setInterval` if the browser does not support it.
	 * @see Crafty.stop
	 */
	init: function(w, h) {
	 ...
	 * @ #Crafty.stop
	 * @category Core
	 * @sign public this Crafty.stop(void) Stops the Enterframe interval and
	 *       removes the stage element.
	 * 
	 ...
	 * @ #Crafty.pause
	 * @comp Core
	 * @sign public this Crafty.pause(void) Pauses the game by stoping the
	 *       EnterFrame event from firing. Pauses automatically when the user
	 *       navigates away from the window. This can be turned off in
	 *       `Crafty.settings`.
	 * @example Have an entity pause the game when it is clicked. ~~~
	 *          button.bind("click", function() { Crafty.pause(); }); ~~~
	 */
	pause: function() {
		if(!this._paused) {
			this.trigger('Pause');
	...
	timer: {
		...
		step: function() {
			loops = 0;
			while((new Date).getTime() > nextGameTick && loops < 20) {
	 ...
	 * @param component# -
	 *            Component to add Creates an entity. Any arguments will be
	 *            applied in the same way `.addComponent()` is applied as a
	 *            quick way to add components.
	 * 
	 * From here, any component added will augment the functionality of that
	 * entity by assigning the properties and methods to that entity.
	 */
	e: function() {
		var id = UID(), craft;
	 ...
	 * @param component -
	 *            Object with the components properties and methods Creates a
	 *            component where the first argument is the ID and the second is
	 *            the object that will be inherited by entities.
	 * 
	 * There is a convention for writing components. For instance properties or
	 * methods that start with an underscore are considered private. A method
	 * called `init` will automatically be called as soon as the component is
	 * added to an entity.
	 * 
	 * A method with the same name as the component ID is considered to be a
	 * constructor and is generally used when data is needed before executing.
	 */
	c: function(id, fn) {
		components[id] = fn;
	},
	 ...
	 * @param data -
	 *            Arbitrary data to pass into the callback as an argument This
	 *            method will trigger every single callback attached to the
	 *            event name. This means every global event and every entity
	 *            that has a callback.
	 * @see Crafty.bind
	 */
	trigger: function(event, data) {
	 ...
	 * @returns ID of the current callback used to unbind Binds to a global
	 *          event. Method will be executed when `Crafty.trigger` is used
	 *          with the event name.
	 ...
	 * @param callbackID -
	 *            ID of the callback
	 * @returns True or false depending on if a callback was unbound Unbind any
	 *          event from any entity or global event.
	 */
	 ...
	 * @ #Crafty.frame
	 * @category Core
	 * @sign public Number Crafty.frame(void) Returns the current frame number
	 */
	frame: function() {
		return frame;
	},
	...
	/**
	 * @ #Crafty.settings
	 * @category Core Modify the inner workings of Crafty through the settings.
	 */
	settings: (function() {
		var states = {},
			callbacks = {};
		...
		return {
			 ...
			 * @sign public void Crafty.settings.register(String settingName,
			 *       Function callback)
			 * @param settingName -
			 *            Name of the setting
			 * @param callback -
			 *            Function to execute when use modifies setting Use this
			 *            to register custom settings. Callback will be executed
			 *            when `Crafty.settings.modify` is used.
			 ...
			 * @comp Crafty.settings
			 * @sign public void Crafty.settings.modify(String settingName, *
			 *       value)
			 * @param settingName -
			 *            Name of the setting
			 * @param value -
			 *            Value to set the setting to Modify settings through
			 *            this method.
			 ...
			 * @param settingName -
			 *            Name of the setting
			 * @returns Current value of the setting Returns the current value
			 *          of the setting.
			 * @see Crafty.settings.register, Crafty.settings.get
			 */
...
HashMap.prototype = {
	...
	search: function(rect,filter) {
		...
		if(filter) {
			...
			// add unique elements to lookup table with the entity ID as unique
			// key
...
/**
 * @ #2D
 * @comp 2D Component for any entity that has a position on the stage.
 */
Crafty.c("2D", {
	/**
	 * @ #.x The `x` position on the stage. When modified, will automatically be
	 *   redrawn. Is actually a getter/setter so when using this value for
	 *   calculations and not modifying it, use the `._x` property.
	 */
	_x: 0,
	/**
	 * @ #.y The `y` position on the stage. When modified, will automatically be
	 *   redrawn. Is actually a getter/setter so when using this value for
	 *   calculations and not modifying it, use the `._y` property.
	 */
	_y: 0,
	/**
	 * @ #.w The width of the entity. When modified, will automatically be
	 *   redrawn. Is actually a getter/setter so when using this value for
	 *   calculations and not modifying it, use the `._w` property.
	 * 
	 * Changing this value is not recommended as canvas has terrible resize
	 * quality and DOM will just clip the image.
	 */
	_w: 0,
	/**
	 * @ #.x The height of the entity. When modified, will automatically be
	 *   redrawn. Is actually a getter/setter so when using this value for
	 *   calculations and not modifying it, use the `._h` property.
	 * 
	 * Changing this value is not recommended as canvas has terrible resize
	 * quality and DOM will just clip the image.
	 */
	_h: 0,
	/**
	 * @ #.z The `z` index on the stage. When modified, will automatically be
	 *   redrawn. Is actually a getter/setter so when using this value for
	 *   calculations and not modifying it, use the `._z` property.
	 * 
	 * A higher `z` value will be closer to the front of the stage. A smaller
	 * `z` value will be closer to the back. A global Z index is produced based
	 * on its `z` value as well as the GID (which entity was created first).
	 * Therefore entities will naturally maintain order depending on when it was
	 * created if same z value.
	 */
	_z: 0,
	/**
	 * @ #.rotation Set the rotation of your entity. Rotation takes degrees in a
	 *   clockwise direction. It is important to note there is no limit on the
	 *   rotation value. Setting a rotation mod 360 will give the same rotation
	 *   without reaching huge numbers.
	 */
	_rotation: 0,
	/**
	 * @ #.alpha Transparency of an entity. Must be a decimal value between 0.0
	 *   being fully transparent to 1.0 being fully opaque.
	 */
	_alpha: 1.0,
	/**
	 * @ #.visible If the entity is visible or not. Accepts a true or false
	 *   value. Can be used for optimization by setting an entities visibility
	 *   to false when not needed to be drawn.
	 * 
	 * The entity will still exist and can be collided with but just won't be
	 * drawn.
	 */
	_visible: true,
	_global: null,
	...
	init: function() {
		...
		// IE9 supports Object.defineProperty
			...
			/*
			 * if no setters, check on every frame for a difference between
			 * this._(x|y|w|h|z...) and this.(x|y|w|h|z)
			 */
			...
			this.bind("EnterFrame", function() {
				// if there are differences between the public and private
				// properties
	 ...
	 * @ #.area
	 * @comp 2D
	 * @sign public Number .area(void) Calculates the area of the entity
	 */
	area: function() {
	 ...
	 * @ #.pos
	 * @comp 2D
	 * @sign public Object .pos(void) Returns the x, y, w, h properties as a
	 *       rect object (a rect object is just an object with the keys _x, _y,
	 *       _w, _h).
	 * 
	 * The keys have an underscore prefix. This is due to the x, y, w, h
	 * properties being merely setters and getters that wrap the properties with
	 * an underscore (_x, _y, _w, _h).
	 */
	pos: function() {
		return {
			_x: (this._x),
	...
	/**
	 * Returns the minimum bounding rectangle. If there is no rotation on the
	 * entity it will return the rect.
	 */
	mbr: function() {
	 ...
	 * @param y -
	 *            Y position of the point Determines whether a point is
	 *            contained by the entity. Unlike other methods, an object can't
	 *            be passed. The arguments require the x and y value
	 */
	isAt: function(x,y) {
	 ...
	 * @param by -
	 *            Amount to move in the specified direction Quick method to move
	 *            the entity in a direction (n, s, e, w, ne, nw, se, sw) by an
	 *            amount of pixels.
	 */
	 ...
	 *            Amount to widen
	 * @param h -
	 *            Amount to increase height Shift or move the entity by an
	 *            amount. Use negative values for an opposite direction.
	 */
	 ...
	 * @param obj -
	 *            Entity(s) to attach Attaches an entities position and rotation
	 *            to current entity. When the current entity moves, the attached
	 *            entity will move by the same amount.
	 * 
	 ...
	 * @param obj -
	 *            The entity to detach. Left blank will remove all attached
	 *            entities Stop an entity from following the current entity.
	 *            Passing no arguments will stop every entity attached.
	 */
	detach: function(obj) {
	 ...
	 * @param offset -
	 *            Combination of center, top, bottom, middle, left and right Set
	 *            the origin point of an entity for it to rotate around.
	 * @example ~~~ this.origin("top left") this.origin("center")
	 *          this.origin("bottom right") this.origin("middle right") ~~~
	 * @see .rotation
	 */
	origin: function(x,y) {
		// text based origin
	...
	/**
	 * Setter method for all 2D properties including x, y, w, h, alpha, rotation
	 * and visible.
	 */
	_attr: function(name,value) {	
		...
		// set the global Z and trigger reorder just incase
			...
			this._global = parseInt(value + Crafty.zeroFill(this[0], 5), 10); // magic
																				// number
																				// 10e5
																				// is
																				// the
...
/**
 * @ #Crafty.Polygon
 * @category 2D Polygon object used for hitboxes and click maps. Must pass an
 *           Array for each point as an argument where index 0 is the x position
 *           and index 1 is the y position.
 * 
 * For example one point of a polygon will look like this: `[0,5]` where the `x`
 * is `0` and the `y` is `5`.
 * 
 * Can pass an array of the points or simply put each point as an argument.
 * 
 * When creating a polygon for an entity, each point should be offset or
 * relative from the entities `x` and `y` (don't include the absolute values as
 * it will automatically calculate this).
 */
...
Crafty.polygon.prototype = {
	 ...
	 *            X position of the point
	 * @param y -
	 *            Y position of the point Method is used to determine if a given
	 *            point is contained by the polygon.
	 * @example ~~~ var poly = new Crafty.polygon([50,0],[100,100],[0,100]);
	 *          poly.containsPoint(50, 50); //TRUE poly.containsPoint(0, 0);
	 *          //FALSE ~~~
	 */
	 ...
	 * @param y -
	 *            Amount to shift the `y` axis Shifts every single point in the
	 *            polygon by the specified amount.
	 * @example ~~~ var poly = new Crafty.polygon([50,0],[100,100],[0,100]);
	 *          poly.shift(5,5); //[[55,5], [105,5], [5,105]]; ~~~
	 */
	shift: function(x,y) {
...
/**
 * @ #Collision
 * @category 2D Component to detect collision between any two convex polygons.
 */
Crafty.c("Collision", {
	 ...
	 *            Constructor takes a polygon to use as the hit area. If left
	 *            empty, will create a rectangle polygon based on the x, y, w, h
	 *            dimensions.
	 * 
	 * This must be called before any .hit() or .onhit() methods.
	 * 
	 * The hit area (polygon) must be a convex shape and not concave for the
	 * collision detection to work.
	 * @example ~~~ this.collision( new Crafty.polygon([50,0], [100,100],
	 *          [0,100]) ); ~~~
	 * @see Crafty.Polygon
	 */
	 ...
	 * @return `false` if no collision. If a collision is detected, returns an
	 *         Array of objects that are colliding. Takes an argument for a
	 *         component to test collision for. If a collision is found, an
	 *         array of every object in collision along with the amount of
	 *         overlap is passed.
	 * 
	 * If no collision, will return false. The return collision data will be an
	 * Array of Objects with the type of collision used, the object collided and
	 * if the type used was SAT (a polygon was used as the hitbox) then an
	 * amount of overlap. ~~~ [{ obj: [entity], type "MBR" or "SAT", overlap:
	 * [number] }] ~~~ `MBR` is your standard axis aligned rectangle
	 * intersection (`.intersect` in the 2D component). `SAT` is collision
	 * between any convex polygon.
	 * @see .onHit, 2D
	 */
	hit: function(comp) {
		var area = this._mbr || this,
	 ...
	 * @ #.onHit
	 * @comp Collision
	 * @sign public this .onHit(String component, Function hit[, Function
	 *       noHit])
	 * @param component -
	 ...
	 *            Creates an enterframe event calling .hit() each time and if
	 *            collision detected will invoke the callback.
	 * @see .hit
	 */
...
/**
 * @ #DOM
 * @category Graphics Draws entities as DOM nodes, specifically `<DIV>`s.
 */
Crafty.c("DOM", {
	/**
	 * @ #._element
	 * @comp DOM The DOM element used to represent the entity.
	 */
	_element: null,
	 ...
	 *            Pass a DOM element to use rather than one created. Will set
	 *            `._element` to this value. Removes the old element.
	 */
	DOM: function(elem) {
	 ...
	 * @triggers Draw Updates the CSS properties of the node to draw on the
	 *           stage.
	 */
	draw: function() {
	 ...
	 * @ #.undraw
	 * @comp DOM
	 * @sign public this .undraw(void) Removes the element from the stage.
	 */
	undraw: function() {
	 ...
	 * @param map -
	 *            Object where the key is the CSS property and the value is CSS
	 *            value Apply CSS styles to the element.
	 * 
	 * Can pass an object where the key is the style property and the value is
	 * style value.
	 * 
	 * For setting one style, simply pass the style as the first argument and
	 * the value as the second.
	 * 
	 * The notation can be CSS or JS (e.g. `text-align` or `textAlign`).
	 * 
	 * To return a value, pass the property.
	 * @example ~~~ this.css({'text-align', 'center', font: 'Arial'});
	 *          this.css("textAlign", "center"); this.css("text-align");
	 *          //returns center ~~~
	 */
...
Crafty.extend({
	/**
	 * @ #Crafty.DOM
	 * @category Graphics Collection of utilities for using the DOM.
	 */
	DOM: {
		/**
		 * @ #Crafty.DOM.window
		 * @comp Crafty.DOM Object with `width` and `height` values representing
		 ...
		 * @returns Object with `x` key being the `x` position, `y` being the
		 *          `y` position Find a DOM elements position including padding
		 *          and border.
		 */
		inner: function(obj) { 
		 ...
		 * @ #Crafty.DOM.getStyle
		 * @comp Crafty.DOM
		 * @sign public Object Crafty.DOM.getStyle(HTMLElement obj, String
		 *       property)
		 * @param obj -
		 *            HTML element to find the style
		 * @param property -
		 *            Style to return Determine the value of a style on an HTML
		 *            element. Notation can be in either CSS or JS.
		 */
		 ...
		 * 
		 * Method will translate x and y positions to positions on the stage.
		 * Useful for mouse events with `e.clientX` and `e.clientY`.
		 */
...
Crafty.extend({
	 ...
	 * @param to -
	 *            Upper bound of the range Returns a random number between (and
	 ...
	 * @ #Crafty.sprite
	 * @category Graphics
	 * @sign public this Crafty.sprite([Number tile], String url, Object map[,
	 ...
	 * @param map -
	 *            Object where the key is what becomes a new component and the
	 *            value points to a position on the sprite map
	 * @param paddingX -
	 *            Horizontal space inbetween tiles. Defaults to 0.
	 * @param paddingY -
	 *            Vertical space inbetween tiles. Defaults to paddingX.
	 *            Generates components based on positions in a sprite image to
	 *            be applied to entities.
	 * 
	 * Accepts a tile size, URL and map for the name of the sprite and it's
	 * position.
	 * 
	 * The position must be an array containing the position of the sprite where
	 * index `0` is the `x` position, `1` is the `y` position and optionally `2`
	 * is the width and `3` is the height. If the sprite map has padding, pass
	 * the values for the `x` padding or `y` padding. If they are the same, just
	 * add one value.
	 * 
	 * If the sprite image has no consistent tile size, `1` or no argument need
	 * be passed for tile size.
	 * 
	 * Entities that add the generated components are also given a component
	 * called `Sprite`.
	 * @see Sprite
	 */
	sprite: function(tile, tileh, url, map, paddingX, paddingY) {
		...
		for(pos in map) {
			...
			/**
			 * @ #Sprite
			 * @category Graphics Component for using tiles in a sprite map.
			 */
			Crafty.c(pos, {
				__image: url,
				__coord: [x,y,w,h],
				 ...
				 * @ #.sprite
				 * @comp Sprite
				 * @sign public this .sprite(Number x, Number y, Number w,
				 *       Number h)
				 * @param x -
				 *            X cell position
				 * @param y -
				 ...
				 *            Width in cells
				 * @param h -
				 *            Height in cells Uses a new location on the sprite
				 *            map as its sprite.
				 * 
				 ...
				 * @ #.crop
				 * @comp Sprite
				 * @sign public this .crop(Number x, Number y, Number w, Number
				 *       h)
				 * @param x -
				 *            Offset x position
				 * @param y -
				 ...
				 *            New width
				 * @param h -
				 *            New height If the entity needs to be smaller than
				 *            the tile size, use this method to crop it.
				 * 
	 ...
	 * @sign public this Crafty.addEvent(Object ctx, HTMLElement obj, String
	 ...
	 * @param callback -
	 *            Method to execute when triggered Adds DOM level 3 events to
	 *            elements. The arguments it accepts are the call context (the
	 *            value of `this`), the DOM element to attach the event to, the
	 *            event name (without `on` (`click` rather than `onclick`)) and
	 ...
	 * @ #Crafty.removeEvent
	 * @category Events, Misc
	 * @sign public this Crafty.removeEvent(Object ctx, HTMLElement obj, String
	 *       event, Function callback)
	 * @param ctx -
	 *            Context of the callback or the value of `this`
	 * @param obj -
	 ...
	 *            Name of the event
	 * @param callback -
	 *            Method executed when triggered Removes events attached by
	 *            `Crafty.addEvent()`. All parameters must be the same that were
	 *            used to attach the event including a reference to the callback
	 *            method.
	 * @see Crafty.addEvent
	 */
	 ...
	 * @param color -
	 *            Modify the background with a color or image This method is
	 *            essentially a shortcut for adding a background style to the
	 *            stage element.
	 */
	...
	/**
	 * @ #Crafty.viewport
	 * @category Stage Viewport is essentially a 2D camera looking at the stage.
	 *           Can be moved which in turn will react just like a camera moving
	 *           in that direction.
	 */
	viewport: {
		width: 0, 
		...
		/**
		 * @ #Crafty.viewport.x
		 * @comp Crafty.viewport Will move the stage and therefore every visible
		 *       entity along the `x` axis in the opposite direction.
		 * 
		 * When this value is set, it will shift the entire stage. This means
		 * that entity positions are not exactly where they are on screen. To
		 * get the exact position, simply add `Crafty.viewport.x` onto the
		 * entities `x` position.
		 */
		_x: 0,
		/**
		 * @ #Crafty.viewport.y
		 * @comp Crafty.viewport Will move the stage and therefore every visible
		 *       entity along the `y` axis in the opposite direction.
		 * 
		 * When this value is set, it will shift the entire stage. This means
		 * that entity positions are not exactly where they are on screen. To
		 * get the exact position, simply add `Crafty.viewport.y` onto the
		 * entities `y` position.
		 */
		_y: 0,
	...
	/**
	 * @ #Crafty.keys
	 * @category Input Object of key names and the corresponding key code. ~~~
	 ...
	 *           INSERT: 45, DELETE: 46, 0: 48, 1: 49, 2: 50, 3: 51, 4: 52, 5:
	 *           53, 6: 54, 7: 55, 8: 56, 9: 57, A: 65, B: 66, C: 67, D: 68, E:
	 *           69, F: 70, G: 71, H: 72, I: 73, J: 74, K: 75, L: 76, M: 77, N:
	 *           78, O: 79, P: 80, Q: 81, R: 82, S: 83, T: 84, U: 85, V: 86, W:
	 *           87, X: 88, Y: 89, Z: 90, NUMPAD_0: 96, NUMPAD_1: 97, NUMPAD_2:
	 *           98, NUMPAD_3: 99, NUMPAD_4: 100, NUMPAD_5: 101, NUMPAD_6: 102,
	 *           NUMPAD_7: 103, NUMPAD_8: 104, NUMPAD_9: 105, MULTIPLY: 106,
	 *           ADD: 107, SUBSTRACT: 109, DECIMAL: 110, DIVIDE: 111, F1: 112,
	 *           F2: 113, F3: 114, F4: 115, F5: 116, F6: 117, F7: 118, F8: 119,
	 *           F9: 120, F10: 121, F11: 122, F12: 123, SHIFT: 16, CTRL: 17,
	 *           ALT: 18, PLUS: 187, COMMA: 188, MINUS: 189, PERIOD: 190 ~~~
	 */
	keys: {
		'BACKSPACE': 8,
        'TAB': 9,
...
/**
 * @ #Crafty.support
 * @category Misc, Core Determines feature support for what Crafty can do.
 */
(function testSupport() {
	 ...
	 * @comp Crafty.support Is `__defineSetter__` supported?
	 */
	support.setter = ('__defineSetter__' in this && '__defineGetter__' in this);
	
	/**
	 * @ #Crafty.support.defineProperty
	 * @comp Crafty.support Is `Object.defineProperty` supported?
	 */
	 ...
	 * @comp Crafty.support Is HTML5 `Audio` supported?
	 */
	support.audio = ('Audio' in window);
	
	/**
	 * @ #Crafty.support.prefix
	 * @comp Crafty.support Returns the browser specific prefix (`Moz`, `o`,
	 *       `ms`, `webkit`).
	 */
	...
	if(match[2]) {
		 ...
		 * @comp Crafty.support Version of the browser
		 */
		support.versionName = match[2];
		
		/**
		 * @ #Crafty.support.version
		 * @comp Crafty.support Version number of the browser as an Integer
		 *       (first number)
		 */
	 ...
	 * @comp Crafty.support Is the `canvas` element supported?
	 */
...
/**
 * @ #Canvas
 * @category Graphics Draws itself onto a canvas. Crafty.canvas() must be called
 *           before hand to initialize the canvas element.
 */
Crafty.c("Canvas", {
	 ...
	 * @ #.draw
	 * @comp Canvas
	 * @sign public this .draw([[Context ctx, ]Number x, Number y, Number w,
	 *       Number h])
	 * @param ctx -
	 ...
	 * @param w -
	 *            Width of the segement to draw
	 * @param h -
	 *            Height of the segment to draw
	 * @triggers Draw Method to draw the entity on the canvas element. Can pass
	 *           rect values for redrawing a segment of the entity.
	 */
...
/**
 * @ #Crafty.canvas
 * @category Graphics Collection of methods to draw on canvas.
 */
Crafty.extend({
	canvas: {
		/**
		 * @ #Crafty.canvas.context
		 * @comp Crafty.canvas This will return the 2D context of the main
		 *       canvas element. The value returned from
		 *       `Crafty.canvas.elem.getContext('2d')`.
		 */
		context: null,
		/**
		 * @ #Crafty.canvas.elem
		 * @comp Crafty.canvas Main Canvas element
		 */
		elem: null,
		 ...
		 * @triggers NoCanvas Creates a `canvas` element inside the stage
		 *           element. Must be called before any entities with the Canvas
		 *           component can be drawn.
		 * 
		 * This method will automatically be called if no
		 * `Crafty.canvas.context` is found.
		 */
		init: function() {
		...
		if(closest) {
			...
			if(e.type === "mousedown") {
				...
				if(this.over !== closest) { // if new mousemove, it is over
					if(this.over) {
						this.over.trigger("MouseOut", e); // if over wasn't
															// null, send
															// mouseout
						this.over = null;
					}
...
/**
 * @ #Mouse
 * @category Input Give entities mouse events such as `mouseover`, `mousedown`,
 *           `mouseout`, `mouseup` and `click`.
 */
Crafty.c("Mouse", {
	init: function() {
		Crafty.mouseObjs++;
	 ...
	 * @param polygon -
	 *            Instance of Crafty.Polygon used to check if the mouse
	 *            coordinates are inside this region
	 * @sign public this .areaMap(Array point1, .., Array pointN)
	 * @param point# -
	 *            Array with an `x` and `y` position to generate a polygon
	 *            Assign a polygon to the entity so that mouse events will only
	 *            be triggered if the coordinates are inside the given polygon.
	 * @see Crafty.Polygon
	 */
...
/**
 * @ #Draggable
 * @category Input Give the ability to drag and drop the entity.
 */
Crafty.c("Draggable", {
	_startX: 0,
	_startY: 0,
	 ...
	 * @ #.stopDrag
	 * @comp Draggable
	 * @sign public this .stopDrag(void) Stop the entity from dragging.
	 *       Essentially reproducing the drop.
	 * @see .startDrag
	 */
	stopDrag: function() {
		Crafty.removeEvent(this, Crafty.stage.elem, "mousemove", this._ondrag);
	 ...
	 * @ #.startDrag
	 * @comp Draggable
	 * @sign public this .startDrag(void) Make the entity follow the mouse
	 *       positions.
	 * @see .stopDrag
	 */
	startDrag: function() {
	 ...
	 * @ #.enableDrag
	 * @comp Draggable
	 * @sign public this .enableDrag(void) Rebind the mouse events. Use if
	 *       `.disableDrag` has been called.
	 * @see .disableDrag
	 */
	enableDrag: function() {		
	 ...
	 * @ #.disableDrag
	 * @comp Draggable
	 * @sign public this .disableDrag(void) Stops entity from being draggable.
	 *       Reenable with `.enableDrag()`.
	 * @see .enableDrag
	 */
...
/**
 * @ #Keyboard
 * @category Input Give entities keyboard events (`keydown` and `keyup`).
 */
Crafty.c("Keyboard", {
	/**
	 * @ #.isDown
	 ...
	 * @param keyCode -
	 *            Key code in `Crafty.keys`. Determine if a certain key is
	 *            currently down.
	 */
	isDown: function(key) {
		if(typeof key === "string") {
...
/**
 * @ #Multiway
 * @category Input Used to bind keys to directions and have the entity move
 *           acordingly
 */
Crafty.c("Multiway", {	
	_speed: 3,
	 ...
	 * @param keyBindings -
	 *            What keys should make the entity go in which direction.
	 *            Direction is specified in degrees Constructor to initialize
	 *            the speed and keyBindings. Component will listen for key
	 *            events and move the entity appropriately.
	 * 
	 * When direction changes a NewDorection event is triggered with an object
	 * detailing the new direction: {x: x_movement, y: y_movement} When entity
	 * has moved on either x- or y-axis a Moved event is triggered with an
	 * object specifieng the old position {x: old_x, y: old_y}
	 * @example ~~~ this.multiway(3, {UP_ARROW: -90, DOWN_ARROW: 90,
	 *          RIGHT_ARROW: 0, LEFT_ARROW: 180}); this.multiway({W: -90, S: 90,
	 *          D: 0, A: 180}); ~~~
	 */
...
/**
 * @ #Fourway
 * @category Input Move an entity in four directions by using the arrow keys or
 *           `W`, `A`, `S`, `D`.
 */
Crafty.c("Fourway", {	
	 ...
	 * @param speed -
	 *            Amount of pixels to move the entity whilst a key is down
	 *            Constructor to initialize the speed. Component will listen for
	 *            key events and move the entity appropriately. This includes
	 *            `Up Arrow`, `Right Arrow`, `Down Arrow`, `Left Arrow` as well
	 *            as `W`, `A`, `S`, `D`.
	 * 
	 * The key presses will move the entity in that direction by the speed
...
/**
 * @ #Twoway
 * @category Input Move an entity in two directions: left or right as well as
 *           jump.
 */
Crafty.c("Twoway", {
	_speed: 3,
	 ...
	 * @param jumpSpeed -
	 *            How high the entity should jump Constructor to initialize the
	 *            speed and power of jump. Component will listen for key events
	 *            and move the entity appropriately. This includes `Up Arrow`,
	 *            `Right Arrow`, `Left Arrow` as well as W, A, D. Used with the
	 *            `gravity` component to simulate jumping.
	 * 
	 * The key presses will move the entity in that direction by the speed
	 * passed in the argument. Pressing the `Up Arrow` or `W` will cause the
	 * entiy to jump.
	 * @see Gravity, Fourway
	 */
...
/**
 * @ #SpriteAnimation
 * @category Animation Used to animate sprites by changing the sprites in the
 *           sprite map.
 */
Crafty.c("SpriteAnimation", {
	 ...
	 * @param fromX -
	 *            Starting `x` position on the sprite map
	 * @param y -
	 *            `y` position on the sprite map. Will remain constant through
	 *            the animation.
	 * @param toX -
	 *            End `x` position on the sprite map
	 * @sign public this .animate(String id, Array frames)
	 * @param frames -
	 *            Array of containing an array with the `x` and `y` values
	 * @sign public this .animate(String id, Number duration[, Number
	 *       repeatCount])
	 * @param duration -
	 *            Play the animation with a duration (in frames) Method to setup
	 *            animation reels or play pre-made reels. Animation works by
	 *            changing the sprites over a duration. Only works for sprites
	 *            built with the Crafty.sprite methods. See the Tween component
	 *            for animation of 2D properties.
	 * 
	 * To setup an animation reel, pass the name of the reel (used to identify
	 * the reel and play it later), and either an array of absolute sprite
	 * positions or the start x on the sprite map, the y on the sprite map and
	 * then the end x on the sprite map.
	 * 
	 * To play a reel, pass the name of the reel and the duration it should play
	 * for (in frames). If you need to repeat the animation, simply pass in the
	 * amount of times the animation should repeat. To repeat forever, pass in
	 * `-1`.
	 * 
	...
	animate: function(id, fromx, y, tox) {
		...
		if(arguments.length < 4 && typeof fromx === "number") {
			...
			this._frame = {
				...
				frameTime: Math.ceil(duration / reel.length), // number of
																// frames
																// inbetween
																// slides
	 ...
	 * @triggers AnimationEnd - Animation is ended Stop any animation currently
	 *           playing.
	 */
	stop: function() {
	 ...
	 * @ #.reset
	 * @comp SpriteAnimation
	 * @sign public this .reset(void) Method will reset the entities sprite to
	 *       its original.
	 */
	reset: function() {
	 ...
	 * @reel reel - Determine if this reel is playing Determines if an animation
	 *       is currently playing. If a reel is passed, it will determine if the
	 *       passed reel is playing.
	 */
...
/**
 * @ #Tween
 * @category Animation Component to animate the change in 2D properties over
 *           time.
 */
Crafty.c("Tween", {
	/**
	 ...
	 * @param duration -
	 *            Duration to animate the properties over (in frames) This
	 *            method will animate a 2D entities properties over the
	 *            specified duration. These include `x`, `y`, `w`, `h`, `alpha`
	 *            and `rotation`.
	 * 
	 * The object passed should have the properties as keys and the value should
	 * be the resulting values of the properties.
	 * @example Move an object to 100,100 and fade out in 200 frames. ~~~
	 *          Crafty.e("2D") .attr({alpha: 1.0, x: 0, y: 0}) .tween({alpha:
	 *          0.0, x: 100, y: 100}, 200) ~~~
	 */
...
/**
 * @ #Color
 * @category Graphics Draw a solid color for the entity
 */
Crafty.c("Color", {
	_color: "",
	ready: true,
	 ...
	 * @param color -
	 *            Color of the rectangle Will create a rectangle of solid color
	 *            for the entity.
	 * 
	 * The argument must be a color readable depending on how it's drawn. Canvas
	 * requires using `rgb(0 - 255, 0 - 255, 0 - 255)` or `rgba()` whereas DOM
	 * can be hex or any other desired format.
	 */
...
/**
 * @ #Tint
 * @category Graphics Similar to Color by adding an overlay of semi-transparent
 *           color.
 * 
...
Crafty.c("Tint", {
	 ...
	 * @param color -
	 *            The color in hexidecimal
	 * @param strength -
	 *            Level of opacity Modify the color and level opacity to give a
	 *            tint on the entity.
	 */
...
/**
 * @ #Image
 * @category Graphics Draw an image with or without repeating (tiling).
 */
Crafty.c("Image", {
	_repeat: "repeat",
	ready: false,
	 ...
	 *            URL of the image
	 * @param repeat -
	 *            If the image should be repeated to fill the entity. Draw
	 *            specified image. Repeat follows CSS syntax (`"no-repeat",
	 *            "repeat", "repeat-x", "repeat-y"`);
	 * 
	 * *Note: Default repeat is `no-repeat` which is different to standard DOM
	 * (which is `repeat`)*
	 * 
	 * If the width and height are `0` and repeat is set to `no-repeat` the
	 * width and height will automatically assume that of the image. This is an
	 * easy way to create an image without needing sprites.
	 * @example Will default to no-repeat. Entity width and height will be set
	 *          to the images width and height ~~~ var ent = Crafty.e("2D, DOM,
	 *          image").image("myimage.png"); ~~~ Create a repeating background.
	 *          ~~~ var bg = Crafty.e("2D, DOM, image") .attr({w:
	 *          Crafty.viewport.width, h: Crafty.viewport.height})
	 *          .image("bg.png", "repeat"); ~~~
	 * @see Crafty.sprite
	 */
...
Crafty.extend({
	 ...
	 * @param init -
	 *            Function execute when scene is played
	 * @sign public void Crafty.scene(String sceneName)
	 * @param sceneName -
	 *            Name of scene to play Method to create scenes on the stage.
	 *            Pass an ID and function to register a scene.
	 * 
	 * To play a scene, just pass the ID. When a scene is played, all entities
	 * with the `2D` component on the stage are destroyed.
	 * 
...
/**
 * Draw Manager will manage objects to be drawn and implement the best method of
 * drawing in both DOM and canvas
 */
	...
	return {
		...
		draw: function draw() {
			...
			for(;i<l;++i) { // loop over every dirty rect
				rect = register[i];
				if(!rect) continue;
				q = Crafty.map.search(rect, false); // search for ents under
													// dirty rect
				
				dupes = {};
				
				// loop over found objects removing dupes and adding to obj
				// array
...
Crafty.extend({
	/**
	 * @ #Crafty.isometric
	 * @category 2D Place entities in a 45deg isometric fashion.
	 */
	isometric: {
		_tile: 0,
		_z: 0,
		 ...
		 * @param tileSize -
		 *            The size of the tiles to place. Method used to initialize
		 *            the size of the isometric placement. Recommended to use a
		 *            size alues in the power of `2` (128, 64 or 32). This makes
		 *            it easy to calculate positions and implement zooming.
		 ...
		 * @comp Crafty.isometric
		 * @sign public this Crafty.isometric.size(Number x, Number y, Number z,
		 *       Entity tile)
		 * @param x -
		 ...
		 * @param tile -
		 *            The entity that should be position in the isometric
		 *            fashion Use this method to place an entity in an isometric
		 *            grid.
...
// **This is canvas only & won't do anything if the browser doesn't support
// it!**
...
Crafty.c("particles", {
	...
	_Particles: {
		presets: {
			...
			// Only applies when fastMode is off, specifies how sharp the
			// gradients are drawn
			sharpness: 20,
			sharpnessRandom: 10,
		...
		addParticle: function () {
			...
			// Take the next particle out of the particle pool we have created
			// and initialize it
		...
		initParticle: function (particle) {
			...
			var newAngle = (this.angle + this.angleRandom * this.RANDM1TO1()) * (Math.PI / 180); // convert
																									// to
																									// radians
			var vector = this.vectorHelpers.create(Math.sin(newAngle), -Math.cos(newAngle)); // Could
																								// move
																								// to
																								// lookup
																								// for
			...
			// internal circle gradient size - affects the sharpness of the
			// radial gradient
...
Crafty.extend({
	/**
	 * @ #Crafty.audio
	 * @category Audio Add sound files and play them. Chooses best format for
	 *           browser support. Due to the nature of HTML5 audio, three types
	 *           of audio files will be required for cross-browser capabilities.
	...
	audio: {
		 ...
		 * @comp Crafty.audio Amount of Audio objects for a sound so overlapping
		 *       of the same sound can occur. More channels means more of the
		 *       same sound playing at the same time.
		 */
		MAX_CHANNELS: 5,
		 ...
		 * @param id -
		 *            A string to reffer to sounds
		 * @param url -
		 *            A string pointing to the sound file
		 * @sign public this Crafty.audio.add(String id, Array urls)
		 * @param urls -
		 *            Array of urls pointing to different format of the same
		 *            sound, selecting the first that is playable
		 * @sign public this Crafty.audio.add(Object map)
		 * @param map -
		 *            key-value pairs where the key is the `id` and the value is
		 *            either a `url` or `urls`
		 * 
		 * Loads a sound to be played. Due to the nature of HTML5 audio, three
		 * types of audio files will be required for cross-browser capabilities.
		 * These formats are MP3, Ogg and WAV.
		 * 
		 * Passing an array of URLs will determine which format the browser can
		 * play and select it over any other.
		 * 
		 * Accepts an object where the key is the audio name and either a URL or
		 * an Array of URLs (to determine which type to use).
		 * 
		 * The ID you use will be how you refer to that sound when using
		 * `Crafty.audio.play`.
		 * 
		 * @example ~~~ //adding audio from an object Crafty.audio.add({ shoot:
		 *          ["sounds/shoot.wav", "sounds/shoot.mp3",
		 *          "sounds/shoot.ogg"],
		 * 
		 * coin: "sounds/coin.mp3" });
		 * 
		 * //adding a single sound Crafty.audio.add("walk", [ "sounds/walk.mp3",
		 * "sounds/walk.ogg", "sounds/walk.wav" ]);
		 * 
		 ...
		 *            A string to reffer to sounds
		 * @param repeatCount -
		 *            Repeat count for the file, where -1 stands for repeat
		 *            forever.
		 * 
		 ...
		 * 
		 * @example ~~~ Crafty.audio.play("walk");
		 * 
		 * //play and repeat forever Crafty.audio.play("backgroundMusic", -1);
		 * ~~~
		...
		play: function(id, repeat) {
			...
			for(;i<l;i++) {
				...
				if(sound.ended || !sound.currentTime) {
					sound.play();
					break;
				} else if(i === l-1) { // if all sounds playing, try stop the
										// last one
					sound.currentTime = 0;
					sound.play();
				}
		 ...
		 * @param id -
		 *            The audio instance added by `Crafty.audio.add`
		 * @param settings -
		 *            An object where the key is the setting and the value is
		 *            what to modify the setting with Used to modify settings of
		 *            the HTML5 `Audio` object. For a list of all the settings
		 *            available, see the [Mozilla
		 *            Documentation](https://developer.mozilla.org/en/XPCOM_Interface_Reference/nsIDOMHTMLMediaElement).
		 */
		...
		/**
		 * @ #Crafty.audio.mute
		 * @sign public this Crafty.audio.mute(void) Mute or unmute every Audio
		 *       instance that is playing. Toggles between pausing or playing
 ...
 * @ #Text
 * @category Graphics
 * @requires DOM Component to draw text inside the body of an entity. Only works
 *           for DOM elements.
 */
Crafty.c("Text", {
	_text: "",
...
Crafty.extend({
	/**
	 * @ #Crafty.assets
	 * @category Assets An object containing every asset used in the current
	 *           Crafty game. The key is the URL and the value is the `Audio` or
	 *           `Image` object.
	 * 
	 * If loading an asset, check that it is in this object first to avoid
	 * loading twice.
	 * @example ~~~ var isLoaded = !!Crafty.assets["images/sprite.png"]; ~~~
	 */
	assets: {},
	 ...
	 * @ #Crafty.loader
	 * @category Assets
	 * @sign public void Crafty.load(Array assets, Function onLoad[, Function
	 ...
	 * @param onProgress -
	 *            Callback when an asset is loaded. Contains information about
	 *            assets loaded
	 * @param onError -
	 *            Callback when an asset fails to load Preloader for all assets.
	 *            Takes an array of URLs and adds them to the `Crafty.assets`
	 *            object.
	 * 
	 ...
	 * 
	 * @example ~~~ Crafty.load(["images/sprite.png", "sounds/jump.mp3"],
	 *          function() { //when loaded Crafty.scene("main"); //go to main
	 *          scene },
	 * 
	 * function(e) { //progress },
	 * 
	...
	load: function(data, oncomplete, onprogress, onerror) {
		...
		for(;i<l;++i) {
			...
			// if there is an error, pass it in the callback (this will be the
			// object that didn't load)
